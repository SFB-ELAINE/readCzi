...
readCziMetadata <- function(input_file = NULL) {
  ...
  df_metadata <- data.frame(
    ...
    "emission_wavelengths_red" =
      ifelse(length(emission_wavelengths[rgb_layers[1]])==0, NA,
             emission_wavelengths[rgb_layers[1]]),
    "emission_wavelengths_green" =
      ifelse(length(emission_wavelengths[rgb_layers[2]])==0, NA,
             emission_wavelengths[rgb_layers[2]]),
    "emission_wavelengths_blue" =
      ifelse(length(emission_wavelengths[rgb_layers[3]])==0, NA,
             emission_wavelengths[rgb_layers[3]]),
    "laser_power_red" =
      ifelse(length(laser_power[rgb_layers[1]])==0, NA,
             laser_power[rgb_layers[1]]),
    "laser_power_green" =
      ifelse(length(laser_power[rgb_layers[2]])==0, NA,
             laser_power[rgb_layers[2]]),
    "laser_power_blue" =
      ifelse(length(laser_power[rgb_layers[3]])==0, NA,
             laser_power[rgb_layers[3]]),
    "laser_attenuation_red" =
      ifelse(length(laser_attenuation[rgb_layers[1]])==0,
             NA, laser_attenuation[rgb_layers[1]]),
    "laser_attenuation_green" =
      ifelse(length(laser_attenuation[rgb_layers[2]])==0,
             NA, laser_attenuation[rgb_layers[2]]),
    "laser_attenuation_blue" =
      ifelse(length(laser_attenuation[rgb_layers[3]])==0,
             NA, laser_attenuation[rgb_layers[3]]),
    "laser_transmission_red" =
      ifelse(length(laser_transmission[rgb_layers[1]])==0,
             NA, laser_transmission[rgb_layers[1]]),
    "laser_transmission_green" =
      ifelse(length(laser_transmission[rgb_layers[2]])==0,
             NA, laser_transmission[rgb_layers[2]]),
    "laser_transmission_blue" =
      ifelse(length(laser_transmission[rgb_layers[3]])==0,
             NA, laser_transmission[rgb_layers[3]]),
    "pixel_time_red" =
      ifelse(length(pixel_time[rgb_layers[1]])==0, NA,
             pixel_time[rgb_layers[1]]),
    "pixel_time_green" =
      ifelse(length(pixel_time[rgb_layers[2]])==0, NA,
             pixel_time[rgb_layers[2]]),
    "pixel_time_blue" =
      ifelse(length(pixel_time[rgb_layers[3]])==0, NA,
             pixel_time[rgb_layers[3]]),
    "photon_conversion_factor_red" =
      ifelse(length(photon_conversion_factor[rgb_layers[1]])==0, NA,
             photon_conversion_factor[rgb_layers[1]]),
    "photon_conversion_factor_green" =
      ifelse(length(photon_conversion_factor[rgb_layers[2]])==0, NA,
             photon_conversion_factor[rgb_layers[2]]),
    "photon_conversion_factor_blue" =
      ifelse(length(photon_conversion_factor[rgb_layers[3]])==0, NA,
             photon_conversion_factor[rgb_layers[3]]),
    "detector_gain_red" =
      ifelse(length(detector_gain[rgb_layers[1]])==0, NA,
             detector_gain[rgb_layers[1]]),
    "detector_gain_green" =
      ifelse(length(detector_gain[rgb_layers[2]])==0, NA,
             detector_gain[rgb_layers[2]]),
    "detector_gain_blue" =
      ifelse(length(detector_gain[rgb_layers[3]])==0, NA,
             detector_gain[rgb_layers[3]]),
    "amplifier_gain_red" =
      ifelse(length(amplifier_gain[rgb_layers[1]])==0, NA,
             amplifier_gain[rgb_layers[1]]),
    "amplifier_gain_green" =
      ifelse(length(amplifier_gain[rgb_layers[2]])==0, NA,
             amplifier_gain[rgb_layers[2]]),
    "amplifier_gain_blue" =
      ifelse(length(amplifier_gain[rgb_layers[3]])==0, NA,
             amplifier_gain[rgb_layers[3]]),
    "amplifier_offset_red" =
      ifelse(length(amplifier_offset[rgb_layers[1]])==0, NA,
             amplifier_offset[rgb_layers[1]]),
    "amplifier_offset_green" =
      ifelse(length(amplifier_offset[rgb_layers[2]])==0, NA,
             amplifier_offset[rgb_layers[2]]),
    "amplifier_offset_blue" =
      ifelse(length(amplifier_offset[rgb_layers[3]])==0, NA,
             amplifier_offset[rgb_layers[3]]),
    "pinhole_diameter_red" =
      ifelse(length(pinhole_diameter[rgb_layers[1]])==0, NA,
             pinhole_diameter[rgb_layers[1]]),
    "pinhole_diameter_green" =
      ifelse(length(pinhole_diameter[rgb_layers[2]])==0, NA,
             pinhole_diameter[rgb_layers[2]]),
    "pinhole_diameter_blue" =
      ifelse(length(pinhole_diameter[rgb_layers[3]])==0, NA,
             pinhole_diameter[rgb_layers[3]]),
    "detection_range_wavelength_red_lower" =
      ifelse(length(rgb_layers[rgb_layers[1]])==0, NA,
    ...
    "detection_range_wavelength_green_lower" =
      ifelse(length(rgb_layers[rgb_layers[2]])==0, NA,
    ...
    "detection_range_wavelength_blue_lower" =
      ifelse(length(rgb_layers[rgb_layers[3]])==0, NA,
